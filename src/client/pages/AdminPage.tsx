...
export function AdminPage() {
  ...
  const revokeSessions = async (user: AdminUser) => {
    if (
      !window.confirm(
        `Sign ${user.fullName} out on every device? They will need to sign in again.`,
      )
    ) {
      return;
    }
      ...
      <section className="mt-6 border border-edge bg-sheet">
        <div className="grid gap-3 border-b border-edge p-4 sm:grid-cols-[minmax(260px,1fr)_180px_180px] sm:p-5">
          ...
          <label>
            ...
            <Select value={state} onChange={(event) => setState(event.target.value as AccountState)}>
                    ...
                    <div className="min-w-0">
                      ...
                      <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-[11px]">
                        <div>
                          ...
                          <dd className="mt-0.5 font-mono text-ink-2">{shortDate(user.createdAt)}</dd>
                        </div>
                        <div>
